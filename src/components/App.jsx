...
function ChampionImage({champ}) {
  ...
  return (
    <div className={`card-container ${champ.name}`}  onClick={ (e) => {
      ...
      if (!pickedCards[0]) {
        ...
        if (pickedCards[0] != pickedCards[1]) {
              ...
              setTimeout(() => {
                const champCards = document.querySelectorAll(`.${pickedCards[0]}`)
      ...
      <img className='card-back' src={`https://ddragon.leagueoflegends.com/cdn/img/champion/loading/${champ.name}_0.jpg`} />
...
function fillArrayWithChamps(numOfCards, setCardArray, nameArray) {
    for (let i = 0; i < numOfCards; i++) {
      ...
      setCardArray(prev => [...prev, {name: nameArray[randomChamp], lives: 2}]);
    }
}
